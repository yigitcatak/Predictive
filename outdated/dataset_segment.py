...
all_files = [f for f in listdir("dataset/original")]
N = 40 #segments of size N
labels = {
...
for channel in range(3,5):
    for name in all_files:
        mat_file = scipy.io.loadmat(f'dataset/original/{name}') 
        ...
        df["label"] = label
        df.to_csv(f'dataset/segmented/{d[channel]}/segmented_{name}.csv', index=False)