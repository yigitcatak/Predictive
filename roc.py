...
Wloc = torch.load('saves/Wloc_10.pt')
model = TiedAutoencoder(50,200,Wloc)
...
stepsize = (maxloss - minloss)/500

results = []
thresholds = [minloss + i*stepsize for i in range(300)]
for threshold in thresholds:
    print(f'step: {int(threshold/stepsize)}/300')
    true_positive = 0
    false_positive = 0
    true_negative = 0
    false_negative = 0
